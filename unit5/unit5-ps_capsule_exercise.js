...
function createCapsule( material, radius, top, bottom, segmentsWidth, openTop, openBottom )
{
	...
	if (!openTop) {
		var topMesh = new THREE.Mesh(sphGeom, material);
		topMesh.position.set(top.x, top.y, top.z);
		capsule.add(topMesh);
	}
	if (!openBottom) {
		var botMesh = new THREE.Mesh(sphGeom, material);
		botMesh.position.set(bottom.x, bottom.y, bottom.z);
		capsule.add(botMesh);
	}
